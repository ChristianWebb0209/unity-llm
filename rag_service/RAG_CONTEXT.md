...
This file is the **single source of truth** for how the **Unity LLM Assistant** works: architecture, tools, conventions, and important paths. **Read this before making changes.**
...
- **Goal**: An AI-native assistant *inside Unity* that:
  - Lives as a **Unity editor plugin**.
  - Talks to a **RAG + LLM backend**.
  - Understands **Unity 4.x** docs, patterns, and **real project code** in:
    - GDScript (`.gd`)
    - C# (`.cs`)
    - Unity shaders (`.gdshader`)
- Current focus:
  - **RAG** (docs + project_code), **context builder** (budgeted blocks, conversation history, repo-index “related files”), **tools**: server-side exploration when `project_root_abs` is set (read_file, list_files, list_directory, search_files, read_import_options), **index-backed tools** (project_structure, find_scripts_by_extends, find_references_to), **editor tools** (file/scene/node edits in Unity), **lint-after-edit** (server lint + auto-fix; one auto follow-up request with lint output so the model can fix without user typing again).
...
- `unity_plugin/addons/unity_ai_assistant/` – Unity editor plugin.
- `unity_knowledge_base/` – Scraped docs + curated code.
- (Removed) ChromaDB vector store used to live at `rag_service/data/chroma_db/` for runtime retrieval.

### 2.1 Plugin folder layout (`unity_plugin/addons/unity_ai_assistant/`)

- **Root**: `plugin.cfg`, `unity_ai_assistant.gd`, `ai_dock.tscn`, `ai_dock.gd` (orchestrator), `settings.gd`, `ai_edit_store.gd`.
...
- **editor/** – Code that changes the Unity editor itself: `editor_decorator.gd` (file/node indicators in script tabs, FileSystem tree, Scene tree).
- **tools/** – Editor tool executor and actions: `editor_tool_executor.gd` (public API: execute, execute_async, preview_file_change; path helpers); `file.gd` (UnityAIFile), `fs.gd` (UnityAIFS), `import.gd` (UnityAIImport), `node.gd` (UnityAINode), `previews.gd` (UnityAIPreviews); **tools/lint/** – `server_lint.gd` (UnityAIServerLint), `lint_capture_logger.gd` (Logger capture), `lint_autofix.gd` (UnityAILintAutofix), `test_lint_capture.gd` (tests). **Run lint capture tests**: in Unity with the plugin enabled, use **Project → Run lint capture tests**; see Output for pass/fail.
...
- Docs pipeline:
  - `rag_service/tools/docs-parser/scrape_unity_docs.py` – crawler → markdown.
- `rag_service/tools/docs-parser/index_docs.py` – optional indexing step (not used for runtime retrieval in this repo).
  - `unity_knowledge_base/docs/4.6/**` – scraped docs.
- Project pipeline:
  - (Removed) `rag_service/scripts/analyze_project.py` – project code analysis/indexing removed from this repo.
  - `unity_knowledge_base/code/demos/<slug>/` – selected important scripts/shaders.
- Repo proximity (structural graph):
  - `unity_plugin/addons/unity_ai_assistant/core/stores/repo_index_store.gd` – client-side JSON cache (`user://unity_ai_assistant/repo_index/<repo_id>.json`) for one-hop related `Assets/` paths.
- Repair memory (lint fixes):
  - `unity_plugin/addons/unity_ai_assistant/core/stores/lint_memory_store.gd` – client-side JSON store (`user://unity_ai_assistant/lint_memory/lint_memory.json`) used to inject `context.extra.lint_repair_memory`.
...
- `GET /health`:
  - Returns `{ "status": "ok" }`.
  - Used by Unity plugin + test scripts to confirm backend is up.
...
- `POST /query_stream`:
  ...
  - Streams back the answer text as plain UTF‑8 chunks so the Unity dock can
...
- **Server-side when `project_root_abs` is set** (LLM sees real results in the same request):
  ...
  - **list_files**: File paths under Assets/, optional extension filter.
...
- **Unity API efficiency**: A short fixed block is injected into the environment (in `main.py` when building `environment_parts`) with tips: _physics_process for movement, cache node refs, signals, move_and_slide/move_and_collide, call_deferred when modifying scene tree from callbacks.
...
- The **`docs` collection** is scraped from the **official Unity 4.x manuals**. It is the
...
- **Docs**: `scrape_unity_docs.py` (BFS crawl → markdown under `unity_knowledge_base/docs/4.6`). Vector indexing into Chroma is no longer used for runtime retrieval in this repo.
- **Project code**: project code analysis/indexing via `analyze_project.py` was removed (see note in §4).

---

## 6. Unity Plugin Dock & UX
...
- **Chat**:
  ...
  - **Tools** checkbox: when checked, the dock uses `POST /query_stream_with_tools`; tool calls with `execute_on_client: true` are run by `UnityAIEditorToolExecutor`. Context usage label near chat tabs (e.g. `Ctx: 2% (792/32768)`).
- **Tabs**:
...
- **Edit History**: Flat ItemList + detail panel (timestamp, summary, files changed, prompt, lint).
  Data comes from local `UnityAIEditStore` persisted to `user://unity_ai_assistant/edit_history/edits.json` (undo/revert is local).
...
- **Lint flow (so the AI always gets real error text)**:
  ...
  2. If the path is not `.gd` (e.g. C#) or capture returns nothing: if RAG backend URL is set, plugin POSTs to backend `/lint` (backend runs `unity --headless --editor --path <project> --script <path> --check-only` and returns stdout/stderr).
  3. If backend URL is empty, plugin runs the same Unity command in a subprocess (`UnityAIServerLint.run_lint_via_unity_subprocess`) and captures output. No third-party linter required. (Third-party options like [unity-diagnostic-list](https://github.com/mphe/unity-diagnostic-list) provide project-wide diagnostics; we don’t integrate them because we need per-file output for the AI fix flow.)
...
- **Local edit store** (`ai_edit_store.gd`): Persisted to `user://unity_ai_assistant_edits.json`. Holds `file_status` (path → status for indicators), `node_status` (scene → node → status), and `events` (timeline, newest first). Used for **editor indicators** and **Revert**.
...
- **Module**: `editor/editor_decorator.gd` (`UnityAIEditorDecorator`) – applies AI edit indicators to the Unity editor UI (script tabs, FileSystem tree, Scene tree). Uses `UnityAIEditStore` for styling constants and status.
- **Styling constants** (`ai_edit_store.gd`): All markers are centralized so “staged” state is consistent across script tabs, FileSystem tree, and Scene tree.
  ...
  - `UnityAIEditStore.strip_markers(s)` strips any of these so decorations can be re-applied without duplicating prefixes.
...
- **Path matching**: FileSystem tree items may store path in metadata; `_normalize_path_for_match()` normalizes slashes and converts project-absolute paths to `Assets/` so `file_status` keys (often `Assets/`) match. Fallback: suffix match on filename when metadata is not a path.
...
- **Settings**: The main Settings UI is the dock’s **Settings tab** (logic in `ui_tabs/settings_tab.gd`; controls are in `ai_dock.tscn` under TabContainer/Settings). An optional popup panel lives in `ui_tabs/settings_panel.gd` + `ui_tabs/settings_panel.tscn` (UnityAISettingsPanel) if you need a separate modal; the primary flow uses the tab.
...
- **Related files**: The Unity plugin may send `context.extra.related_res_paths` (one-hop `Assets/` dependencies). The backend embeds those files into the “Related files” context block; it does not require SQLite-based repo indexing.
...
- **Storage**: Cached JSON per project stored by the Unity plugin:
  - `unity_plugin/addons/unity_ai_assistant/core/stores/repo_index_store.gd`
  - `user://unity_ai_assistant/repo_index/<repo_id>.json`
- **Indexing**: On-demand. When a file is active, the store reads it and extracts `Assets/...` references, then uses those references for one-hop related selection.
- **Context flow**: The plugin injects `context.extra.related_res_paths` (list of `Assets/` paths). The backend embeds those files’ text into the “Related files” context block.
...
- **Streaming + tools**: `query_stream_with_tools` streams answer, then `__TOOL_CALLS__` + JSON, then `__USAGE__`. Backend-resolved tools (read_file, list_files, list_directory, search_files, read_import_options, project_structure, find_scripts_by_extends, find_references_to when `project_root_abs` set) are already executed; only `execute_on_client: true` tools are run in Unity after the stream.
- **project_root_abs**: Plugin sends it in `context.extra`. Backend uses it for server-side file/exploration tools and for embedding the active/related file context it receives from the plugin. Open Unity from the **project folder** (e.g. `unity_plugin`), not repo root.
...
- **Editor decorations**: Use `EditorInterface.get_file_system_dock()` / `get_script_editor()`; match script tabs via `get_open_scripts()[i].resource_path`; `call_deferred("_apply_editor_decorations")` so docks exist. Paths: normalize to `Assets/`; `ProjectSettings.globalize_path("Assets/")` for project root.
